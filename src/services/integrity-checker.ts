...
export class IntegrityChecker {
  ...
  async getSourceHealth(): Promise<SourceHealthReport> {
    ...
    const sourceStats: SourceHealthStat[] = sources.map(s => {
      const daysSinceLastDoc = s.last_document_at
        ? Math.floor((now.getTime() - new Date(s.last_document_at).getTime()) / (1000 * 60 * 60 * 24))
        : null;

      const isStale = daysSinceLastDoc === null || daysSinceLastDoc > staleThresholdDays;
      const hasContent = s.document_count > 0;

      return {
        sourceId: s.id,
        sourceName: s.name,
        baseTrust: s.base_trust,
        lastDocumentAt: s.last_document_at,
        daysSinceLastDocument: daysSinceLastDoc,
        documentCount: s.document_count,
        claimCount: s.claim_count,
        avgExtractionConfidence: s.avg_extraction_confidence,
        status: !hasContent ? 'empty' : isStale ? 'stale' : 'active',
      };
    });
    ...
    return {
      ...
      activeSources: sourceStats.filter(s => s.status === 'active').length,
      staleSources: sourceStats.filter(s => s.status === 'stale').length,
      emptySources: sourceStats.filter(s => s.status === 'empty').length,
      sources: sourceStats,
    };
  }
  ...
  async getPipelineStats(): Promise<PipelineStats> {
    ...
    return {
      timestamp: new Date(),
      sources: row.total_sources,
      documents: row.total_documents,
      snippets: row.total_snippets,
      claims: row.total_claims,
      evidence: row.total_evidence,
      conflictGroups: row.total_conflict_groups,
      claimsWithMetrics: row.claims_with_metrics,
      claimsWithoutMetrics: row.total_claims - row.claims_with_metrics,
      activeConflicts: row.active_conflicts,
      pendingReviews: row.pending_review,
      metricsCompletion: row.total_claims > 0
        ? Math.round((row.claims_with_metrics / row.total_claims) * 100)
        : 100,
    };
  }
  ...
  async autoFix(options: {
    fixCounts?: boolean;
    fixOrphanedClaims?: boolean;
    fixInvalidLinks?: boolean;
    ...
    if (options.fixCounts !== false) {
      const count = await this.fixConflictGroupCounts();
      if (count > 0) {
        result.fixes.push({ type: 'conflict_group_counts', count });
        result.totalFixed += count;
      }
    }

    if (options.fixOrphanedClaims) {
      const count = await this.deleteOrphanedClaims();
      if (count > 0) {
        result.fixes.push({ type: 'orphaned_claims', count });
        result.totalFixed += count;
      }
    }

    if (options.fixInvalidLinks !== false) {
      const count = await this.clearInvalidFieldLinks();
      if (count > 0) {
        result.fixes.push({ type: 'invalid_field_links', count });
        result.totalFixed += count;
      }
    }
...
export interface PipelineStats {
  ...
  claimsWithMetrics: number;
  claimsWithoutMetrics: number;
  activeConflicts: number;
  pendingReviews: number;
  metricsCompletion: number;
}

export interface AutoFixResult {
  timestamp: Date;
  fixes: Array<{ type: string; count: number }>;
  totalFixed: number;
}
