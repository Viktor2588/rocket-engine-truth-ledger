...
import { runAIExtraction } from '../jobs/ai-extract.js';
 ...
 * Get all facts for an entity with pagination
 * Query params: truth_min, limit, offset
 */
router.get('/entities/:entityId/facts', asyncHandler(async (req, res) => {
  const { entityId } = req.params;
  // Support both truth_min (frontend) and truth_slider (legacy) parameters
  const slider = parseFloat(req.query.truth_min as string ?? req.query.truth_slider as string) || 0.5;
  const limit = parseInt(req.query.limit as string) || 50;
  const offset = parseInt(req.query.offset as string) || 0;

  const response = await factResolver.getEntityFacts(entityId, slider, { limit, offset });
...
/**
 * GET /source-health
 * Get detailed source health statistics
 */
router.get('/source-health', asyncHandler(async (_req, res) => {
  const integrityChecker = new IntegrityChecker();
  const report = await integrityChecker.getSourceHealth();
  res.json(report);
}));
...
router.get('/pipeline-stats', asyncHandler(async (_req, res) => {
  const integrityChecker = new IntegrityChecker();
  const stats = await integrityChecker.getPipelineStats();
  res.json(stats);
}));

/**
 * GET /integrity
 * Run integrity checks and return report
 */
router.get('/integrity', asyncHandler(async (_req, res) => {
  const integrityChecker = new IntegrityChecker();
  const report = await integrityChecker.runAllChecks();
  res.json(report);
}));
 ...
 */
router.post('/integrity/auto-fix', asyncHandler(async (req, res) => {
  const { fixCounts = true, fixOrphanedClaims = false, fixInvalidLinks = true } = req.body || {};
...
/**
 * GET /alerts
 * Get active system alerts based on health and integrity checks
 */
router.get('/alerts', asyncHandler(async (req, res) => {
  ...
  // Check source health
  const health = await integrityChecker.getSourceHealth();
  ...
  if (staleSources.length > 0) {
    alerts.push({
      ...
      message: `${staleSources.length} sources have not been updated in 30+ days`,
      details: {
        staleSources: staleSources.map(s => ({
          name: s.sourceName,
          lastDocument: s.lastDocumentAt,
          daysSinceUpdate: s.daysSinceLastDocument,
        })),
      },
      timestamp,
    });
  ...
  if (emptySources.length > 0) {
    alerts.push({
      level: 'info',
      source: 'source_health',
      message: `${emptySources.length} configured sources have no documents`,
      details: {
  ...
  // Unscored claims alert
  if (stats.claimsWithoutMetrics > 0 && stats.claims > 0) {
    const pct = Math.round((stats.claimsWithoutMetrics / stats.claims) * 100);
    if (pct > 10) {
      alerts.push({
        level: pct > 30 ? 'warning' : 'info',
        source: 'pipeline',
        message: `${stats.claimsWithoutMetrics} claims (${pct}%) are missing truth metrics`,
        details: {
          claimsWithoutMetrics: stats.claimsWithoutMetrics,
          totalClaims: stats.claims,
          recommendation: 'Run the scorer job: pnpm job:score',
        },
        timestamp,
      });
    }
  ...
  if (stats.pendingReviews > 10) {
    alerts.push({
      level: stats.pendingReviews > 50 ? 'warning' : 'info',
      source: 'review_queue',
      message: `${stats.pendingReviews} items are pending human review`,
      details: {
  ...
  if (stats.activeConflicts > 0) {
    alerts.push({
      level: 'info',
      source: 'conflicts',
      message: `${stats.activeConflicts} active data conflicts detected`,
      details: {
  ...
  // Run quick integrity checks for critical issues
  const integrity = await integrityChecker.runAllChecks();
...
const JOB_DEFINITIONS = [
  ...
  {
    id: 'extract',
    name: 'Claim Extraction (Regex)',
    description: 'Extract claims using regex patterns - fast but less accurate',
    category: 'processing',
    estimatedDuration: '5-30 minutes',
  ...
  },
  {
    id: 'ai_extract',
    name: 'AI Extraction',
    description: 'Extract claims using Claude AI - understands context, variants, and conditions',
    category: 'processing',
...
async function executeJob(jobId: string, runId: string, sql: ReturnType<typeof getConnection>, parentJobId?: string) {
  ...
  try {
    ...
    switch (jobId) {
      ...
      case 'ai_extract': {
        updateProgress(5, 100, 'Initializing AI extractor...');
        log('Starting AI-enhanced claim extraction...');
        const aiResult = await runAIExtraction({
          limit: 50, // Process 50 snippets per run (AI is slower but more accurate)
          checkCancelled,
          onProgress: (current, total, message) => {
            // Scale progress from 5-100 (5% for init)
            const scaledProgress = 5 + Math.floor((current / total) * 95);
            updateProgress(scaledProgress, 100, message);
          },
        });
        recordsSynced = aiResult.claimsCreated + aiResult.evidenceCreated;
        log(`AI extraction complete: ${aiResult.claimsCreated} claims, ${aiResult.evidenceCreated} evidence, ${aiResult.errors.length} errors`);
        updateProgress(100, 100, `AI extracted ${aiResult.claimsCreated} claims from ${aiResult.snippetsProcessed} snippets`);
        break;
      }
